...
window.placeOrder = async function placeOrder({ name, email, plan, amount }) {
  ...
  const announcement = `New order: ${plan} by ${payload.customer_name}`;
  const { error: aerr } = await sb.from('announcements').insert([{ message: announcement }]);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      const res = await window.placeOrder({ name, email, plan, amount });
      if (!res.success) {
